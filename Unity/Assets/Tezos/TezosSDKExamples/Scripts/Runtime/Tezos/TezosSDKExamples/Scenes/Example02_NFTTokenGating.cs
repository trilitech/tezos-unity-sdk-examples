...
namespace TezosSDKExamples.Scenes
{
    ...
    public class Example02_NFTTokenGating : Example02_Parent
    {
        ...
        protected override async UniTask OnCheckForNft01ButtonClicked()
        {
            ...
            {
                result =
                    $"The wallet address <b>{activeWalletAddress}</b> has "
                    + $"the NFT with address <b>{demoNFTAddress}</b> and tokenId <b>{demoTokenId}</b>.\n\n";
            }
            else
            {
                result =
                    $"The wallet address <b>{activeWalletAddress}</b> does NOT have "
                    + $"the NFT with address <b>{demoNFTAddress}</b> and tokenId <b>{demoTokenId}</b>.\n\n";
            }
        ...
        protected override async UniTask OnCheckForNft02ButtonClicked()
        {
            ...
            {
                result =
                    $"The wallet address <b>{demoWalletAddress}</b> has "
                    + $"the NFT with address <b>{demoNFTAddress}</b> and tokenId <b>{demoTokenId}</b>.\n\n";
            }
            else
            {
                result =
                    $"The wallet address <b>{demoWalletAddress}</b> does NOT have "
                    + $"the NFT with address <b>{demoNFTAddress}</b> and tokenId <b>{demoTokenId}</b>.\n\n";
            }
        ...
        protected override async UniTask OnListAllNftsButtonClicked()
        {
            ...
            // Display Result
            string result =
                $"The wallet address <b>{demoWalletAddress}</b> has {tokenBalances.Count} NFTs.\n\n";
            ...
            foreach (TokenBalance tokenBalance in tokenBalances)
            {
                ...
                result +=
                    $" • tokenId = {tokenBalance.tokenId}, balance = {tokenBalance.balance}\n";
            }
